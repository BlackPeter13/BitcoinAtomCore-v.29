#!/usr/bin/env bash
# no-docker-install.sh — offline installer for BCA-Core29 WITHOUT Docker
# Target: freshly installed Ubuntu 22.04 Server (jammy), no network required.
# Builds natively on the host system (no container overhead).
...
#   sudo ./no-docker-install.sh [--no-tests] [-j N]
set -euo pipefail
...
log() { echo "[nodocker] $*"; }
die() { echo "[nodocker] ERROR: $*" >&2; exit 1; }

[ "$(id -u)" = "0" ] || die "run as root (sudo ./no-docker-install.sh)"

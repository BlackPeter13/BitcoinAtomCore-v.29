    ...
    while (pcursor->Valid()) {
        ...
        if (pcursor->GetKey(key) && key.first == DB_BLOCK_INDEX) {
            ...
            if (pcursor->GetValue(diskindex)) {
                ...
                // Atom: restore proof-of-stake index data (stake modifier
                // chain, PoS flags, PoW height).
                ...
                // Atom: PoS headers carry no proof of work.
...
CBlockIndex* BlockManager::AddToBlockIndex(const CBlockHeader& block, CBlockIndex*& best_header)
{
    ...
    if (miPrev != m_block_index.end()) {
        ...
        // Atom: track the proof-of-work height (PoS blocks don't advance it)
        // and carry the header consensus flags into the index.
...
bool BlockManager::ReadBlock(CBlock& block, const FlatFilePos& pos) const
{
    ...
    // Check the header (Atom: PoS headers carry no proof of work)

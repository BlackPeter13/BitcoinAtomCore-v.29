 ...
 * - VARINT(block time the containing transaction was included at; Atom only,
 *   zero for pre-fork entries read from older chainstates is not supported,
 *   use -reindex after upgrading)
...
public:
    ...
    //! Atom: block time the containing transaction was included at.
    //! Used for coinstake age checks.
    uint32_t nTime{0};

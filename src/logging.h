...
namespace BCLog {
    ...
    enum LogFlags : CategoryMask {
        ...
        STAKEMODIFIER = (CategoryMask{1} << 29), //!< Atom: proof-of-stake modifier selection
        COINAGE = (CategoryMask{1} << 30), //!< Atom: coinstake coin-age calculation
        COINSTAKE = (CategoryMask{1} << 31), //!< Atom: wallet coinstake search
        ALL         = ~NONE,
    };
    enum class Level {

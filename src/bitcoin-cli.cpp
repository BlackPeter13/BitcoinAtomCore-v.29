...
static void SetupCliArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-convertaddress=<bitcoin_address>", "Convert a Bitcoin mainnet address into Bitcoin Atom address format and exit.", ArgsManager::ALLOW_ANY, OptionsCategory::CLI_COMMANDS);
}
...
{
    // Convert a legacy Bitcoin address (base58 P2PKH/P2SH) to Atom format.
    // Manual version-byte mapping: the params-based decoder cannot switch
    // networks inside bitcoin-cli.
    SelectParams(ChainType::MAIN);
    const std::string old_address{gArgs.GetArg("-convertaddress", "")};
    std::vector<unsigned char> decoded;
    if (!DecodeBase58Check(old_address, decoded, 70) || decoded.size() != 21) {
        tfm::format(std::cerr, "Invalid Bitcoin address: %s\n", old_address);
        return EXIT_FAILURE;
    }
    CTxDestination dest;
    if (decoded[0] == 0x00) {
        dest = PKHash(uint160(std::vector<unsigned char>(decoded.begin() + 1, decoded.end())));
    } else if (decoded[0] == 0x05) {
        dest = ScriptHash(uint160(std::vector<unsigned char>(decoded.begin() + 1, decoded.end())));
    } else {
        tfm::format(std::cerr, "Not a P2PKH/P2SH Bitcoin address: %s\n", old_address);
        return EXIT_FAILURE;
    }
    // Encoded with the active (Atom mainnet) parameters selected above.
...
static int AppInitRPC(int argc, char* argv[])
{
    ...
    // Atom: offline Bitcoin -> Atom address conversion.
    if (gArgs.IsArgSet("-convertaddress")) {
        if (gArgs.GetChainType() != ChainType::MAIN) {
            tfm::format(std::cerr, "Error: Only mainnet addresses can be converted.\n");
            return EXIT_FAILURE;
        }

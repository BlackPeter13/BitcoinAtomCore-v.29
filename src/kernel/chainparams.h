...
public:
    ...
    const MessageStartChars& MessageStartLegacy() const { return pchMessageStartLegacy; }
    uint16_t GetDefaultPort(bool bootstrapping = false) const { return bootstrapping ? nBitcoinDefaultPort : nDefaultPort; }
    ...
    const std::vector<uint8_t>& FixedSeeds(bool bootstrapping = false) const { return bootstrapping ? vFixedBootstrapSeeds : vFixedSeeds; }
    ...
    /** Whether the block at the given height predates Atom consensus rules */
    bool isLegacyBlock(uint32_t nHeight) const { return nHeight < (uint32_t)consensus.BCAHeight; }
...
protected:
    ...
    MessageStartChars pchMessageStartLegacy;
    uint16_t nDefaultPort;
    uint16_t nBitcoinDefaultPort{0};
    ...
    CBlock genesis;
    std::vector<uint8_t> vFixedBootstrapSeeds;
...
/**
 * Return the chain parameters with Bitcoin address format. Used for
 * address conversion of pre-fork (legacy) outputs.
 */

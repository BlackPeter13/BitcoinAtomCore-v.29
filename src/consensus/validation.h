...
{
    // Atom: pre-fork legacy blocks serialize the 80-byte header (no nFlags).
    size_t no_witness_size{::GetSerializeSize(TX_NO_WITNESS(block))};
    size_t with_witness_size{::GetSerializeSize(TX_WITH_WITNESS(block))};
    if (!block.IsNewFormatBlock()) {
        assert(no_witness_size >= sizeof(block.nFlags));
        assert(with_witness_size >= sizeof(block.nFlags));
        no_witness_size -= sizeof(block.nFlags);
        with_witness_size -= sizeof(block.nFlags);
    }
    return no_witness_size * (WITNESS_SCALE_FACTOR - 1) + with_witness_size;
}

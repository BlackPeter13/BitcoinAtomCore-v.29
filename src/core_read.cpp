...
/** Atom: decode a pre-fork (legacy) hex block with the 80-byte header. */
bool DecodeHexBlkLegacy(CBlock& block, const std::string& strHexBlk)
{
    if (!IsHex(strHexBlk))
        return false;
    ...
    try {
        CBlockHeader hdr;
        ssBlock >> Using<LegacyBlockHeaderFormatter>(hdr);
        static_cast<CBlockHeader&>(block) = hdr;
        ssBlock >> TX_WITH_WITNESS(block.vtx);
...
util::Result<int> SighashFromStr(const std::string& sighash)
{
    static const std::map<std::string, int> map_sighash_values = {
        ...
        // Atom: fork-id commits (required post-fork, valid pre-fork).

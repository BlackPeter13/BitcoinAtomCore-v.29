...
private:
    ...
    // Atom: pre-fork (Bitcoin) peers speak the legacy magic during bootstrap.
    const MessageStartChars m_legacy_magic_bytes;
    ...
    bool m_uses_legacy_magic GUARDED_BY(m_recv_mutex){false}; // Atom: peer spoke legacy magic
...
public:
    ...
    /** Atom: whether this peer connected with the legacy (pre-fork) magic. */
    bool UsesLegacyMagic() const EXCLUSIVE_LOCKS_REQUIRED(!m_recv_mutex)
    {
        AssertLockNotHeld(m_recv_mutex);
        return WITH_LOCK(m_recv_mutex, return m_uses_legacy_magic);
    }

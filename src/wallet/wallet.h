...
public:
    ...
    //! Atom: unlock restricted to proof-of-stake minting (set by walletpassphrase ... mintonly).
    bool fWalletUnlockMintOnly GUARDED_BY(cs_wallet){false};
    ...
    /** Atom: search wallet UTXOs for a stake kernel and build an unsigned
     * coinstake transaction (legacy wallets; requires -txindex). See
     * stake.cpp for the parameter contract. */
    bool CreateCoinStake(ChainstateManager& chainman, unsigned int nBits, int64_t nSearchInterval, CMutableTransaction& txNew, uint32_t& nCoinStakeTime, CAmount& nPosReward) EXCLUSIVE_LOCKS_REQUIRED(cs_wallet);

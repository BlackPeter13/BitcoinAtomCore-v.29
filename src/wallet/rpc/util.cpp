...
void EnsureWalletIsUnlocked(const CWallet& wallet)
{
    ...
    // Atom: mint-only unlocks do not permit spending.
    if (WITH_LOCK(wallet.cs_wallet, return wallet.fWalletUnlockMintOnly)) {
        throw JSONRPCError(RPC_WALLET_UNLOCK_NEEDED, "Error: Wallet is unlocked for staking only. Unlock fully to send funds.");
    }
}

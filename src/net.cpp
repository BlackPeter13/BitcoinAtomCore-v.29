...
V1Transport::V1Transport(const NodeId node_id) noexcept
    : m_magic_bytes{Params().MessageStart()},
      m_legacy_magic_bytes{Params().MessageStartLegacy()},
      m_node_id{node_id}
{
    LOCK(m_recv_mutex);
    Reset();
...
int V1Transport::readHeader(Span<const uint8_t> msg_bytes)
{
    ...
    // Check start string, network magic. Atom: also accept the legacy
    // (pre-fork) magic so bootstrap peers can feed pre-fork history.
    if (hdr.pchMessageStart != m_magic_bytes) {
        if (hdr.pchMessageStart == m_legacy_magic_bytes) {
            m_uses_legacy_magic = true;
        } else {

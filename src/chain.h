...
public:
    ...
    //! Atom: count of proof-of-work blocks up to and including this entry.
    //! Used for PoW retargeting that skips proof-of-stake blocks.
    int nPowHeight{0};
    ...
    //! Atom: block index flags (proof-of-stake / entropy / modifier bits).
    uint32_t nFlags{0};
    //! Atom: hash modifier for proof-of-stake.
    uint64_t nStakeModifier{0};
    //! Atom: checksum of index; in-memory only.
    uint32_t nStakeModifierChecksum{0};
    //! Atom: proof-of-stake hash (PoS blocks only).
    uint256 hashProofOfStake{};
    //! Atom: coinbase output paid to the minter of a proof-of-stake block.
    COutPoint outStakeReward{};
    ...
    //! Atom: efficiently find the proof-of-work ancestor at the given
    //! PoW height (skips proof-of-stake entries).
    const CBlockIndex* GetPowAncestor(int powHeight, const Consensus::Params& params) const;
...
public:
    ...
    SERIALIZE_METHODS(CDiskBlockIndex, obj)
    {
        ...
        // Atom: proof-of-stake index data. Serialized unconditionally:
        // loading an index written by an older format requires -reindex.
        READWRITE(obj.nFlags);
        READWRITE(obj.nStakeModifier);
        if (obj.IsProofOfStake()) {
            READWRITE(obj.hashProofOfStake);
            READWRITE(obj.outStakeReward);
        }
        READWRITE(VARINT_MODE(obj.nPowHeight, VarIntMode::NONNEGATIVE_SIGNED));
    }

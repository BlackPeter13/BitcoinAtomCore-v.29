...
bool CheckInputScripts(const CTransaction& tx, TxValidationState& state,
                       ...
                       std::vector<CScriptCheck>* pvChecks = nullptr, bool enforce_forkid = false)
...
// Atom: whether the hard-fork rules apply at the block following pindexPrev.
...
static bool CheckInputsFromMempoolAndCache(const CTransaction& tx, TxValidationState& state,
                ...
                ValidationCache& validation_cache, bool enforce_forkid = false)
                EXCLUSIVE_LOCKS_REQUIRED(cs_main, pool.cs)
{
    AssertLockHeld(cs_main);
    AssertLockHeld(pool.cs);
    ...
    return CheckInputScripts(tx, state, view, flags, /* cacheSigStore= */ true, /* cacheFullScriptStore= */ true, txdata, validation_cache, /* pvChecks= */ nullptr, enforce_forkid);
}
...
bool MemPoolAccept::PreChecks(ATMPArgs& args, Workspace& ws)
{
    ...
    // Atom: also enforce coin inclusion-time (no future-coin spends past the fork).
    const uint32_t nMempoolTime{static_cast<uint32_t>(count_seconds(GetTime<std::chrono::seconds>()))};
    if (!Consensus::CheckTxInputs(tx, state, m_view, m_active_chainstate.m_chain.Height() + 1, ws.m_base_fees,
                                  nMempoolTime, m_active_chainstate.m_chainman.GetConsensus().BCAHeight)) {
...
bool MemPoolAccept::PolicyScriptChecks(const ATMPArgs& args, Workspace& ws)
{
    ...
    // Atom: post-fork, signatures must carry the fork id (checked against the
    // next block height, as mempool transactions mine in the future).
    const bool enforce_forkid{IsBCAHardForkEnabled(m_active_chainstate.m_chain.Tip(), m_active_chainstate.m_chainman.GetConsensus())};
    if (!CheckInputScripts(tx, state, m_view, scriptVerifyFlags, true, false, ws.m_precomputed_txdata, GetValidationCache(), /* pvChecks= */ nullptr, enforce_forkid)) {
...
{
    // Atom: halvings are counted on proof-of-work height, so proof-of-stake
    // blocks do not accelerate the subsidy schedule. Callers connecting or
    // mining PoW blocks must pass the index/height PoW counter.
...
std::optional<std::pair<ScriptError, std::string>> CScriptCheck::operator()() {
    ...
    checker.SetEnforceForkId(m_enforce_forkid);
...
bool CheckInputScripts(const CTransaction& tx, TxValidationState& state,
                       ...
                       bool cacheFullScriptStore, PrecomputedTransactionData& txdata,
                       ValidationCache& validation_cache,
                       std::vector<CScriptCheck>* pvChecks, bool enforce_forkid)
{
    ...
    // Atom: forkid enforcement changes the verdict, so it is part of the key.
    const unsigned char forkid_byte{enforce_forkid ? 1 : 0};
    hasher.Write(UCharCast(tx.GetWitnessHash().begin()), 32).Write((unsigned char*)&flags, sizeof(flags)).Write(&forkid_byte, 1).Finalize(hashCacheEntry.begin());
    ...
    for (unsigned int i = 0; i < tx.vin.size(); i++) {
        ...
        // Verify signature
        CScriptCheck check(txdata.m_spent_outputs[i], tx, validation_cache.m_signature_cache, i, flags, cacheSigStore, &txdata, enforce_forkid);
        if (pvChecks) {
...
static bool IsBCAHardForkEnabled(int nHeight, const Consensus::Params& params) {
...
bool IsBCAHardForkEnabled(const CBlockIndex* pindexPrev, const Consensus::Params& params) {
    ...
    return IsBCAHardForkEnabled(pindexPrev->nHeight + 1, params);
}

bool IsBCAHardForkEnabledForCurrentBlock(ChainstateManager& chainman) {
    AssertLockHeld(cs_main);
    return IsBCAHardForkEnabled(chainman.ActiveTip(), chainman.GetConsensus());
}

/** Minimum fee rate (sat/kB) required of the coinstake transaction in a PoS
 * block. Matches BCA's use of the wallet default transaction fee. */
static constexpr CAmount COINSTAKE_MIN_FEE_RATE{1000};

/** Proof-of-stake block reward as a function of coin age. BCA pays fees only. */
...
/**
 * Coin age of a coinstake's inputs, in coin-days, resolved through the block
 * store (the previous block header supplies the input maturity timestamps).
 */
static bool GetCoinAge(ChainstateManager& chainman, const CTransaction& tx, const CCoinsViewCache& view, uint64_t& nCoinAge, const Consensus::Params& params, uint32_t nTime)
{
    AssertLockHeld(cs_main);
    ...
    for (const CTxIn& txin : tx.vin)
    {
        ...
        const COutPoint &prevout = txin.prevout;
        if (!view.GetCoin(prevout))
            continue;  // previous transaction not in main chain

        uint256 hashBlock;
        CTransactionRef txPrev = node::GetTransaction(/*block_index=*/nullptr, /*mempool=*/nullptr, prevout.hash, hashBlock, chainman.m_blockman);
        if (!txPrev) {
            LogError("%s() : tx missing in tx index in GetCoinAge()", __func__);
            return false;
        }
        const CBlockIndex* pindexFrom = chainman.m_blockman.LookupBlockIndex(hashBlock);
        if (!pindexFrom) {
            LogError("%s() : block index missing in GetCoinAge()", __func__);
            return false;
        }
        CBlock blockFrom;
        if (!chainman.m_blockman.ReadBlock(blockFrom, *pindexFrom)) {
            LogError("%s() : block unreadable in GetCoinAge()", __func__);
            return false;
        }
        const CBlockHeader& header = blockFrom;
        if (nTime < header.GetBlockTime())
            return false;  // timestamp violation

        if (txPrev->GetHash() != prevout.hash) {
            LogError("%s() : txid mismatch in GetCoinAge()", __func__);
            return false;
        }
        ...
        // BCA counted age in cent-seconds (CENT = 0.01 coin).
        bnCentSecond += arith_uint256(nValueIn) * (nTime - header.GetBlockTime()) / (COIN / 100);

        LogDebug(BCLog::COINAGE, "coin age nValueIn=%-12lld nTimeDiff=%d bnCentSecond=%s\n", nValueIn, nTime - header.GetBlockTime(), bnCentSecond.ToString());
    }

    arith_uint256 bnCoinDay = bnCentSecond * (COIN / 100) / COIN / (24 * 60 * 60);
    LogDebug(BCLog::COINAGE, "coin age bnCoinDay=%s\n", bnCoinDay.ToString());
    nCoinAge = bnCoinDay.GetLow64();
    return true;
}

// Atom: view-less GetCoinAge for wallet staking (builds a short-lived view
// over the coins tip; callers must not hold cs_main).
bool GetCoinAge(ChainstateManager& chainman, const CTransaction& tx, uint64_t& nCoinAge, uint32_t nTime)
{
    LOCK(cs_main);
    CCoinsViewCache view(&chainman.ActiveChainstate().CoinsTip());
    return GetCoinAge(chainman, tx, view, nCoinAge, chainman.GetConsensus(), nTime);
}

// These checks can only be done when all previous blocks have been added.
static bool PoSContextualBlockChecks(ChainstateManager& chainman, const CTxMemPool* mempool, const CBlock& block, BlockValidationState& state, CBlockIndex* pindex, const Consensus::Params& consensus, bool fJustCheck)
{
    AssertLockHeld(cs_main);
    uint256 hashProofOfStake;
    if (block.IsProofOfStake()) {
        if (pindex->nHeight <= consensus.BCAHeight + consensus.BCAInitLim) {
            return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "bad-pos-height",
                strprintf("%s: adding proof-of-stake block was rejected due to its height (%d): %s", __func__, pindex->nHeight, block.GetHash().ToString()));
        }

        if (block.vtx.size() < 2) {
            return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "bad-pos-malformed",
                "ConnectBlock() : incorrect PoS block");
        }

        // pos: verify hash target and signature of coinstake tx
        if (!CheckProofOfStake(state, chainman, mempool, block.vtx[1], block.nBits, hashProofOfStake, block.GetBlockTime())) {
    ...
    if (!ComputeNextStakeModifier(chainman, pindex, consensus, nStakeModifier, fGeneratedStakeModifier)) {
        return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "bad-stake-modifier",
            "ConnectBlock() : ComputeNextStakeModifier() failed");
    }
    ...
    if (!pindex->SetStakeEntropyBit(nEntropyBit)) {
        return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "bad-stake-entropy",
            "ConnectBlock() : SetStakeEntropyBit() failed");
    }
    ...
    if (!CheckStakeModifierCheckpoints(pindex->nHeight, nStakeModifierChecksum)) {
        return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "bad-stake-modifier-checkpoint",
            strprintf("ConnectBlock() : rejected by stake modifier checkpoint height=%d", pindex->nHeight));
    }
                               ...
                               CCoinsViewCache& view, bool fJustCheck)
{
    ...
    // Atom: proof-of-stake contextual checks (kernel hash, stake modifier).
    if (!PoSContextualBlockChecks(m_chainman, m_mempool, block, state, pindex, params.GetConsensus(), fJustCheck))
        return false;
    ...
    // Atom: post-fork, input signatures must carry the fork id.
    const bool enforce_forkid{IsBCAHardForkEnabled(pindex->pprev, m_chainman.GetConsensus())};
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++)
    {
        ...
        if (!tx.IsCoinBase())
        {
            ...
            if (!Consensus::CheckTxInputs(tx, tx_state, view, pindex->nHeight, txfee, pindex->GetBlockTime(), params.GetConsensus().BCAHeight)) {
            ...
            // Atom: consider second transaction in PoS block as a coinstake transaction
            if (block.IsProofOfStake() && i == 1) {
                CFeeRate minFeeRate{COINSTAKE_MIN_FEE_RATE};
                if (txfee < minFeeRate.GetFee(GetSerializeSize(TX_WITH_WITNESS(tx)))) {
                    state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "bad-txns-coinstake-fee-too-small",
                                  strprintf("ConnectBlock() : %s stake fee too small", tx.GetHash().ToString()));
                    break;
                }

                uint64_t nCoinAge{0};
                if (!GetCoinAge(m_chainman, tx, view, nCoinAge, params.GetConsensus(), block.GetBlockTime())) {
                    state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "bad-txns-coinstake-age",
                                  strprintf("ConnectBlock() : %s unable to get coin age for coinstake", tx.GetHash().ToString()));
                    break;
                }
        ...
        if (!tx.IsCoinBase())
        {
            ...
            TxValidationState tx_state;
            if (fScriptChecks && !CheckInputScripts(tx, tx_state, view, flags, fCacheResults, fCacheResults, txsdata[i], m_chainman.m_validation_cache, parallel_script_checks ? &vChecks : nullptr, enforce_forkid)) {
                // Any transaction validation failure in ConnectBlock is a block consensus failure
                state.Invalid(BlockValidationResult::BLOCK_CONSENSUS,
    ...
    if (block.IsProofOfWork()) {
        CAmount blockReward = nFees + GetBlockSubsidy(pindex->nPowHeight, params.GetConsensus());
        if (block.vtx[0]->GetValueOut() > blockReward && state.IsValid()) {
            state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "bad-cb-amount",
                          strprintf("coinbase pays too much (actual=%d vs limit=%d)", block.vtx[0]->GetValueOut(), blockReward));
        }
    } else {
        if (block.vtx[0]->GetValueOut() > nFees + posReward && state.IsValid()) {
            state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "bad-cs-amount",
                          strprintf("ConnectBlock() : coinstake pays too much (actual=%d vs limit=%d)",
...
{
    // Check proof of work matches claimed amount (Atom: PoS headers carry no PoW)
...
bool CheckBlock(const CBlock& block, BlockValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    // Size limits. Atom: pre-fork legacy blocks serialize the 80-byte header
    // (no nFlags); the default serialization includes it, so subtract it.
    size_t nBlockSize = ::GetSerializeSize(TX_NO_WITNESS(block));
    if (!block.IsNewFormatBlock()) {
        assert(nBlockSize >= sizeof(block.nFlags));
        nBlockSize -= sizeof(block.nFlags);
    }
    if (block.vtx.empty() || block.vtx.size() * WITNESS_SCALE_FACTOR > MAX_BLOCK_WEIGHT || nBlockSize * WITNESS_SCALE_FACTOR > MAX_BLOCK_WEIGHT)
    ...
    // Atom: check proof-of-stake block signature
...
{
    if (block.GetHash() == consensusParams.hashGenesisBlock || !block.IsProofOfStake())
...
bool SignBlock(CBlock& block, const SigningProvider& keystore)
{
        ...
        return false;
    {
        ...
        if (key.GetPubKey() != pubKey)
            return false;
        return key.Sign(block.GetHash(), block.vchBlockSig, /*use_rfc6979=*/false);
    }
    return false;
}
...
static bool ContextualCheckBlockHeader(const CBlockHeader& block, BlockValidationState& state, BlockManager& blockman, const ChainstateManager& chainman, const CBlockIndex* pindexPrev) EXCLUSIVE_LOCKS_REQUIRED(::cs_main)
{
    ...
    // Check proof of work (Atom: dispatch on proof-of-stake flag)
    const Consensus::Params& consensusParams = chainman.GetConsensus();
    if (block.nBits != GetNextWorkRequired(pindexPrev, &block, consensusParams, block.IsProofOfStake()))
        return state.Invalid(BlockValidationResult::BLOCK_INVALID_HEADER, "bad-diffbits", "incorrect proof of work/proof of stake");
    ...
    // Atom: proof-of-stake blocks are not allowed inside the post-fork
    // initialization window.
    if (nHeight <= consensusParams.BCAHeight + consensusParams.BCAInitLim && block.IsProofOfStake())
        return state.Invalid(BlockValidationResult::BLOCK_INVALID_HEADER, "pos-restriction-initlim", "adding pos block is restricted within initlim");
...
bool ChainstateManager::AcceptBlockHeader(const CBlockHeader& block, BlockValidationState& state, CBlockIndex** ppindex, bool min_pow_checked)
{
    ...
    if (hash != GetConsensus().hashGenesisBlock) {
        ...
        if (!CheckBlockHeader(block, state, GetConsensus(), /*fCheckPOW=*/!block.IsProofOfStake())) {
...
bool ChainstateManager::AcceptBlock(const std::shared_ptr<const CBlock>& pblock, BlockValidationState& state, CBlockIndex** ppindex, bool fRequested, const FlatFilePos* dbp, bool* fNewBlock, bool min_pow_checked)
{
    ...
    // Atom: block serialization format must match the height (pre-fork
    // blocks are legacy, post-fork blocks carry consensus flags).
    if (block.IsNewFormatBlock() != IsBCAHardForkEnabled(pindex->nHeight, params.GetConsensus())) {
        LogError("AcceptBlock(): block format does not match expected at height %d (%s format)\n",
            pindex->nHeight, (block.IsNewFormatBlock() ? "new" : "old"));
        return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "bad-blk-format",
            "block format does not match expected for height");
    }

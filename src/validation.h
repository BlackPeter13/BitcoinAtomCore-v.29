...
/** Atom: compute the coin age (value x days) consumed by a coinstake's inputs. */
bool GetCoinAge(ChainstateManager& chainman, const CTransaction& tx, uint64_t& nCoinAge, uint32_t nTime);

/** Atom: proof-of-stake reward for a coinstake with the given coin age (fees only). */
...
private:
    ...
    // Atom: post-fork forkid enforcement, wired from consensus with height.
    bool m_enforce_forkid{false};

public:
    CScriptCheck(const CTxOut& outIn, const CTransaction& txToIn, SignatureCache& signature_cache, unsigned int nInIn, unsigned int nFlagsIn, bool cacheIn, PrecomputedTransactionData* txdataIn, bool enforce_forkid = false) :
        m_tx_out(outIn), ptxTo(&txToIn), nIn(nInIn), nFlags(nFlagsIn), cacheStore(cacheIn), txdata(txdataIn), m_signature_cache(&signature_cache), m_enforce_forkid(enforce_forkid) { }
...
/** Atom: verify the block signature of a proof-of-stake block against the
 * coinstake's key. Pre-fork (PoW) blocks are valid with an empty signature. */
bool CheckBlockSignature(const CBlock& block, const Consensus::Params& consensusParams);

/** Atom: sign a proof-of-stake block with the coinstake key from the wallet. */

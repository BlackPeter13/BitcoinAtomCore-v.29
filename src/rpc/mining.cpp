 ...
 */
static UniValue GetNetworkHashPS(int lookup, int height, const CChain& active_chain, const Consensus::Params& consensus) {
    ...
    if (lookup == -1)
        lookup = pb->nPowHeight % consensus.DifficultyAdjustmentIntervalPow() + 1;
    ...
    // Atom: measure over proof-of-work blocks only.
    while (pb0 && pb0->IsProofOfStake())
        pb0 = pb0->pprev;
    if (pb0 == nullptr)
        return 0;
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    LOCK(cs_main);
    return GetNetworkHashPS(self.Arg<int>("nblocks"), self.Arg<int>("height"), chainman.ActiveChain(), chainman.GetConsensus());
},
    };
}
    ...
    return RPCHelpMan{"getmininginfo",
                ...
                RPCResult{
                    RPCResult::Type::OBJ, "", "",
                    {
                        ...
                        {RPCResult::Type::NUM, "search-interval", /*optional=*/true, "The interval covered by the last coinstake search in seconds (Atom only, present after staking search runs)"},
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    // Atom: coinstake search coverage (see BlockAssembler::CreateNewPoSBlock).
    obj.pushKV("search-interval", node::nLastCoinStakeSearchInterval);
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    if (!request.params[0].isNull())
    {
        ...
        if (strMode == "proposal")
        {
            ...
            BlockValidationState state;
            // Atom: proposals may be unsigned (PoS templates are signed by the
            // minter only after assembly).
            TestBlockValidity(state, chainman.GetParams(), chainman.ActiveChainstate(), block, chainman.m_blockman.LookupBlockIndex(block.hashPrevBlock), /*fCheckPOW=*/false, /*fCheckMerkleRoot=*/true, /*fCheckSig=*/false);
...
static RPCHelpMan submitblock()
{
    ...
    return RPCHelpMan{"submitblock",
        ...
        "See https://en.bitcoin.it/wiki/BIP_0022 for full specification.\n",
        {
            ...
            {"legacy", RPCArg::Type::BOOL, RPCArg::Default{false}, "indicates the block uses the pre-fork legacy format (80-byte header, Atom only)"},
        },
        {
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    const bool fLegacyFormat{request.params[2].isNull() ? false : request.params[2].get_bool()};
    if (!(fLegacyFormat ? DecodeHexBlkLegacy(block, request.params[0].get_str())
    ...
    ChainstateManager& chainman = EnsureAnyChainman(request.context);
    {
        ...
        if (pindex) {
            ...
            // Atom: mark post-fork blocks so they serialize with consensus flags.
            if (pindex->nHeight + 1 >= chainman.GetConsensus().BCAHeight) {

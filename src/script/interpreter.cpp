...
template <class T>
uint256 SignatureHash(const CScript& scriptCode, const T& txTo, unsigned int nIn, int32_t nHashType, const CAmount& amount, SigVersion sigversion, const PrecomputedTransactionData* cache, SigHashCache* sighash_cache, const int forkid)
{
    assert(nIn < txTo.vin.size());

    // Atom: signatures carrying SIGHASH_FORKID commit to the fork id in the
    // high bits of the sighash type and always use the BIP143 digest.
    ...
    // (Atom: forkid sighashes bypass the cache — the cache index does not
    // distinguish forkid digests from legacy ones.)
    if (sighash_cache && !UsesForkId(nHashType) && sighash_cache->Load(nHashType, scriptCode, ss)) {
    ...
    if (sighash_cache != nullptr && !UsesForkId(nHashType)) {
...
bool GenericTransactionSignatureChecker<T>::CheckECDSASignature(const std::vector<unsigned char>& vchSigIn, const std::vector<unsigned char>& vchPubKey, const CScript& scriptCode, SigVersion sigversion) const
{
    ...
    // Atom: post-fork consensus requires the fork-id sighash flag.
    if (m_enforce_forkid && !(nHashType & SIGHASH_FORKID)) return false;

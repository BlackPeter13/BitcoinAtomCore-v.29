...
/** Fork IDs (Atom: replay protection across the hard fork) */
enum
{
    FORKID_BCC = 0,
    FORKID_BCA = 93,
...
template <class T>
uint256 SignatureHash(const CScript& scriptCode, const T& txTo, unsigned int nIn, int32_t nHashType, const CAmount& amount, SigVersion sigversion, const PrecomputedTransactionData* cache = nullptr, SigHashCache* sighash_cache = nullptr, const int forkid = FORKID_IN_USE);
...
private:
    ...
    // Atom: when set, ECDSA signatures must carry SIGHASH_FORKID (post-fork
    // replay protection). Wired from consensus with the block height; default
    // off so wallet/tests keep stock semantics.
...
public:
    ...
    void SetEnforceForkId(bool enforce) { m_enforce_forkid = enforce; }

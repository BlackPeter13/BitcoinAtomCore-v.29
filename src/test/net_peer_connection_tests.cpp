...
BOOST_FIXTURE_TEST_CASE(test_addnode_getaddednodeinfo_and_connection_detection, PeerTest)
{
    ...
    connman->SetMsgProc(peerman.get()); // Atom: mirror production wiring (init.cpp); teardown DeleteNode needs it
    NodeId id{0};
    ...
    connman.reset(); // Atom: stop connman while peerman (msgproc) is alive, mirroring production shutdown order
}

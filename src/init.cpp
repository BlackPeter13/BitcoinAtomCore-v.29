...
void SetupServerArgs(ArgsManager& argsman, bool can_listen_ipc)
{
    ...
    argsman.AddArg("-bootstrap", "Atom: connect to the Bitcoin P2P network to download pre-fork blockchain history.", ArgsManager::ALLOW_ANY, OptionsCategory::CONNECTION);
    argsman.AddArg("-skiphardforkibd", "Atom: skip initial block download latch when reaching the hard-fork block.", ArgsManager::ALLOW_ANY, OptionsCategory::CONNECTION);
...
bool AppInitMain(NodeContext& node, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    // Atom: bootstrap/sync modes.
    fBCABootstrapping = args.GetBoolArg("-bootstrap", false);
    fSkipHardforkIBD = args.GetBoolArg("-skiphardforkibd", false);
    if (fBCABootstrapping) {
        LogPrintf("Atom bootstrap mode enabled: syncing pre-fork history from Bitcoin peers\n");
    }
